...
scenario outlines. Use `Given a file "<path>" is rewritten with content "<text>"`
or `Given a temp file "<name>" is rewritten with content "<text>"` to force a
write when filesystem-watcher behavior is the feature under test. Both forms
also accept doc-string content.
...
`workbench.action.closeAllEditors` step instead fails before changing dirty
editors and reports their filenames. This preserves unsaved work and the
dirty-state regression signal without leaving VS Code waiting for user input.
